...
double perimeterPolygon(vector<pt> &p) {
    double ans = 0.0;
    for (int i = 0, n = p.size(); i < n; i++) {
        ans += abs(p[i] - p[(i + 1) % n]);
    }
...
/** End Convex **/
const int NUMP = 365 * 5;
int P, K;
double L;
vector<pt> pts;
vector<pt> hull;
pt binSearch(int iter) {
    double l = 0, r = 1e3 + 5;
    pt dir = {1, 0};
    dir = rot(dir, (iter / (NUMP + 0.)) * 2 * PI);
    for (int i = 0; i < 30; i++) {
        double mid = (l + r) / 2;
        vector<pt> cur = pts;
        cur.push_back(dir * mid);
        cur = convexHull(cur);
        if (perimeterPolygon(cur) > L) {
            r = mid;
        } else {
            l = mid;
        }
...
signed main() {
    ...
    for (int t = 0; t < P; t++) {
        ...
        for (int i = 0; i < NUMP; i++) {
            hull.push_back(binSearch(i));
        }
        hull = convexHull(hull);
        cout << t + 1 << ' ';
        cout << fixed << setprecision(20) << areaPolygon(hull) << endl;
    }
}