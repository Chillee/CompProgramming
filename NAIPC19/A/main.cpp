...
int circleCircle(pt o1, ld r1, pt o2, double r2, pair<pt, pt> &out) {
    pt d = o2 - o1;
    ld d2 = sq(d);
...
int tangents(pt o1, ld r1, pt o2, double r2, bool inner, vector<pair<pt, pt>> &out) {
    if (inner)
        r2 = -r2;
    pt d = o2 - o1;
...
signed main() {
    ...
    for (int i = 0; i < N; i++) {
        ld curArea = 0;
        for (int j = 1; j <= K; j++) {
            curArea += areaTriangle(pts[i], pts[(i + j + 1) % N], pts[(i + j) % N]);
            ans += curArea * prob[j];
        }
    }
    ld offset = ((N - K) * ((N - K) - 1.0)) / (N * (N - 1.0));
    cout << fixed << setprecision(20) << abs(areaPolygon(pts)) - ans * offset << endl;
}