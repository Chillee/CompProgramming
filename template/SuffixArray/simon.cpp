...
void test(const string &s, int alpha) {
    ...
    iota(all(suffixes), 0);
    sort(all(suffixes), [&](int a, int b) { return s.substr(a) < s.substr(b); });
    ...
    rep(i, 0, sz(s)) {
        int j = 0;
        while (max(j + suffixes[i], suffixes[i + 1]) < sz(s) && s[j + suffixes[i]] == s[j + suffixes[i + 1]]) j++;
        lcp[i + 1] = j;
    }
...
void count_sort(vector<pli> &b, int bits) { // (optional)
    ...
    for (int it = 0; it < 2; it++) {
        ...
        for (int i = 0; i < sz(b); i++) q[(b[i].first >> move) & mask]++;
        partial_sum(q.begin(), q.end(), w.begin() + 1);
        vector<pli> res(b.size());
        for (int i = 0; i < sz(b); i++) res[w[(b[i].first >> move) & mask]++] = b[i];
        swap(b, res);
    }
}
struct SuffixArray {
    ...
    SuffixArray(const string &_s) : s(_s + '\0') {
        ...
        for (int moc = 0;; moc++) {
            ...
            a[b[0].second] = 0;
            for (int i = 1; i < N; i++) a[b[i].second] = a[b[i - 1].second] + (b[i - 1].first != b[i].first);
...
signed compare() {
    ...
    for (int iter = 0; iter < 5; iter++) {
        ...
        vector<array<int, 2>> res;
        {
            ...
            auto lcp = kactl.lcp();
            rep(i, 0, sz(S) + 1) res.push_back({kactl.a[i], lcp[i]});
        }
        {
            timeit x("MIT");
            SuffixArray sa(S);
            // cout << sa.sa[100] << endl;
            rep(i, 0, sz(S) + 1) { assert((res[i] == array<int, 2>{sa.sa[i], sa.lcp[i]})); }
        }
    }
    return 0;
}
...
void perf2() {
    vi str;
    rep(i, 0, 300'000) str.push_back(1 + (rand() % 100'000));
    str.push_back(0);
    ll res = 0;
    rep(i, 0, 30) {
