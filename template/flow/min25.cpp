...
class DoublyLinkedList {
  ...
  public:
    ...
    template <typename Func> void erase_all(int first, int last, Func f) {
        for (int i = first; i <= last; ++i) {
            for (int h = head[i]; h >= 0; h = nodes[h].next)
                f(h);
            head[i] = -1;
        }
    }
...
template <typename CapType, typename TotalCapType, bool UseGlobal = false, bool UseGap = false> class HLPP {
  private:
    static constexpr TotalCapType inf = numeric_limits<TotalCapType>::max();
    struct Edge {
        int to, rev;
        CapType cap;
    };
  ...
  public:
    HLPP(int N) : N(N), G(N), que(N), list(N), dlist(N) {}
  ...
  private:
    void global_relabel(int t) {
        ...
        for (que[qt++] = t; qh < qt;) {
            ...
            for (auto &e : G[u])
                if (height[e.to] == N && G[e.to][e.rev].cap > 0) {
...
template <class T> inline void scan_int(T &p) {
    static char c;
    while (isspace(c = getchar_unlocked()))
        ;
    for (p = c - '0'; isdigit(c = getchar_unlocked()); p = p * 10 + c - '0')
        ;
}
const int MAXN = 5005;
HLPP<int, int> *hlpp;
