...
template <typename cap_t, typename excess_t, bool global_relabeling = true, bool min_cut_only = false,
          bool shuffle_edges = false>
class Push_Relabel {
  public:
    struct Edge {
        int to, rev;
                        ...
                        if (__builtin_expect(u_cur == g_pos[u + 1], false)) {
                            ...
                            if (__builtin_expect(!--h_cnt[hi] && hi < n, false)) {
                                ...
                                for (int j = hi; j <= hi_all; ++j) {
                                    for (auto &f : buck_all[j])
                                        if (!min_cut_only || h[f] < n) {
                                            --h_cnt[h[f]];
                                            h[f] = n + 1;
                                        }
                                    buck_all[j].clear();
