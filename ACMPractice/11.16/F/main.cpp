...
int count(pt a, pt b) {
    ...
    for (auto e : edges) {
        ...
        //         pt noise = {dx, dy};
        //         noise = noise * EPS;
        //         if (inters(e.first[0] + noise, e.first[1] + noise, a, b).size() > 0) {
        //             found[e.second] = true;
        //         }
        //     }
        // }
        cnt++;
        // cout << segSeg(e.first[0], e.first[1], a, b) << endl;
        pt diff2 = (e.first[1] - e.first[0]);
...
int check(pt p) {
    ...
    for (auto a : edges) {
        for (auto b : edges) {
            ...
            if (orientedAngle({0, 0}, la.v, lb.v) < 10 * EPS)
                continue;
            ld x = abs(la.dist(p));
            ld y = abs(lb.dist(p));
            if (y == 0 || x == 0)
                continue;
            ld theta = asin((ld)1.0 / (sqrt(1 + pow(y / x, (ld)2.0 / (ld)3.0))));
            vector<ld> dirs = {theta, -theta, -theta + PI / 2, theta + PI / 2, -theta + PI, theta + PI, -theta + 3 * PI / 2, 3 * PI / 2 + theta};
            for (auto d : dirs) {
                pt dir = {cos(d), sin(d)};
                dir = (dir / abs(dir)) * L;
                // ans = max(ans, count(p, p + dir));
                pt outa, outb;
                pt diff2 = (a.first[1] - a.first[0]);
                pt diff3 = (b.first[1] - b.first[0]);
                if (properInter(p, p + dir, a.first[0], a.first[1] + diff2 * EPS, outa) && properInter(p, p - dir, b.first[0] - diff3 * EPS, b.first[1] + diff3 * EPS, outb)) {
