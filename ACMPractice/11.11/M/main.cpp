...
double EPS = 1e-8;
double eval(double x, double a, double b, double c, double d) { return (x * a + (1 - x) * c) * (x * b + (1 - x) * d); }
double ternary(double a, double b, double c, double d) {
    double l = 0, r = 1;
    for (int i = 0; i < 50; i++) {
        double mid = (l + r) / 2;
        double mv = eval(mid, a, b, c, d);
        double lv = eval(mid - EPS, a, b, c, d);
        double rv = eval(mid + EPS, a, b, c, d);
        if (lv <= mv && mv <= rv) {
            l = mid;
        } else if (lv >= mv && mv >= rv) {
            r = mid;
...
signed main() {
    ...
    N = pts.size();
    for (int i = 0; i <= N; i++) {
        ans = max(ans, pts[i % N].x * pts[i % N].y);
        ans = max(ans, ternary(pts[i % N].x, pts[i % N].y, pts[(i + 1) % N].x, pts[(i + 1) % N].y));
    }
