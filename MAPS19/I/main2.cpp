...
struct line {
    ...
    // array<pt, 2> getPts() { return {proj({0, 0}), proj(v)}; } // returns two arbitrary points on the line
    // - these require T = ld
...
ld linAngle(pt a, pt b) { return orientedAngle({0, 0}, {-1, 0}, a - b); }
bool operator<(line &a, line &b) {
    int val = sgn(linAngle(a.v, pt{0, 0}) - linAngle(b.v, pt{0, 0}));
    if (val != 0)
        return val == 1;
    return b.c <= a.c;
}

bool operator==(line &a, line &b) { return sgn(linAngle(a.v, {0, 0}) - linAngle(b.v, {0, 0})) == 0; }

// line que[MAX_N_BORDER];

bool check(line &a, line &b, line &me) {
    pt is{0, 0};
    inter(a, b, is);
    return sgn(me.side(is)) > 0;
}
vector<pt> convexIntersection(vector<line> &border) {
    int qh = 0, qt = 0;
    sort(border.begin(), border.end());
    border.resize(unique(border.begin(), border.end()) - border.begin());
    vector<line> que(border.size());
    ...
    vector<pt> pts(sz);
    for (int i = 0; i < sz; i++)
        inter(que[i + qh], que[(i + 1) % sz + qh], pts[i]);
    return pts;
}

ld halfPlaneIntersection(vector<line> border) {
    border.push_back(line{pt{-LARGE, -LARGE}, pt{LARGE, -LARGE}});
    border.push_back(line{pt{LARGE, -LARGE}, pt{LARGE, LARGE}});
    border.push_back(line{pt{LARGE, LARGE}, pt{-LARGE, LARGE}});
    border.push_back(line{pt{-LARGE, LARGE}, pt{-LARGE, -LARGE}});
    auto res = convexIntersection(border);
    return areaPolygon(res);
}
...
line shiftLine(line l, ld x) {
    pt pp = perp(l.v);
    pp = pp / abs(pp);
    pp = pp * x;
    return l.translate(pp);
}
ld binSearch(ld l, ld r, int iter) {
    ld mid = (l + r) / 2.0;
    if (iter == 50)
        return mid;
    vector<line> cur;
    ...
    ld area = halfPlaneIntersection(res);
    if (abs(area) <= 0) {
