...
set<pt> inters(pt a, pt b, pt c, pt d) { // Intersections between AB and CD. |S|: 1->point, 0->none, 2->segment
    pt out;
...
double segPoint(pt a, pt b, pt p) { // Distance between AB and P
    if (a != b) {
        line l(a, b);
...
double segSeg(pt a, pt b, pt c, pt d) { // Distance between AB and CD
    pt dummy;
...
signed main() {
    ...
    cin.tie(0);
    vector<pt> poly = {{0, 0}, {3, 1}, {3, 2}};
    cout << maxDistSq(poly) << endl;
    // cout << inPolygon(poly, {2, 1}, false) << endl;
    // return 0;
    int ans = 0;
    for (int x = 0; x <= N; x++) {
        for (int y = 0; y <= x; y++) {
            for (int x2 = 0; x2 <= x; x2++) {
                for (int y2 = 0; y2 <= x2; y2++) {
                    vector<pt> poly = {{0, 0}, {x, y}, {x2, y2}};
                    poly = convexHull(poly);
                    if (poly.size() != 3)
                        continue;
                    if (poly[0] != pt{0, 0} || poly[1] != pt{x, y} || poly[2] != pt{x2, y2})
                        continue;
                    bool works = true;
                    int top = max_element(poly.begin(), poly.end()) - poly.begin();
                    for (int i = 0; i <= N; i++) {
                        if (!works)
                            break;
                        for (int j = 0; j <= N; j++) {
