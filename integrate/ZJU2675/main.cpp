...
void polarSort(pt o, vector<pt> &v) {
    sort(v.begin(), v.end(),
         [o](pt v, pt w) { return make_tuple(half(v - o), 0) < make_tuple(half(w - o), cross(v - o, w - o)); });
}
/* Point End */
/* Line Start */
struct line {
...
bool inConvex(vector<pt> &l, pt p, bool strict = true) {
    ...
    if (det(l[0], l[a], p) > 0 || det(l[0], l[b], p) < 0 ||
        (strict && (det(l[0], l[a], p) == 0 || det(l[0], l[b], p) == 0)))
        return false;
    while (abs(a - b) > 1) {
        c = (a + b) / 2;
        if (det(l[0], l[c], p) > 0)
...
bool isIn(pt x) { return x.x >= X1 && x.x <= X2 && x.y >= Y1 && x.y <= Y2; }
double f(double x) {
    pt cur = {cos(x) * r + xc, sin(x) * r + yc};
    for (auto i : segs) {
        auto res = inters(pt{xc, yc}, cur, i.first, i.second);
        if (res.size() > 0) {
            cur = *(res.begin());
            break;
        }
...
signed main() {
    ...
    while (cin >> xc) {
        cin >> yc >> r;
        cin >> X1 >> Y1 >> X2 >> Y2;
        segs.push_back({{X1, Y1}, {X1, Y2}});
        segs.push_back({{X1, Y1}, {X2, Y1}});
        segs.push_back({{X1, Y2}, {X2, Y2}});
        segs.push_back({{X2, Y1}, {X2, Y2}});
    }
    cout << integrate(f, 0, 2 * PI) << endl;
}