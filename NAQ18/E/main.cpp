...
signed main() {
    ...
    for (auto a : apples) {
        for (auto b : apples) {
            ...
            tangents(a, 1, b, 1, false, res);
            tangents(a, 1, b, 1, true, res);
            for (auto tl : res) {
                if (sq(tl.first - tl.second) < 1e-9)
                    continue;
                line l = line(tl.first, tl.second);
                int curAns = 0;
                for (auto i : apples) {
                    pair<pt, pt> t;
                    curAns += (circleLine(i, 1 + 1e-9, l, t) > 0);
                }
                ans = max(curAns, ans);
            }
        }
    ...
    for (int i = 1; i < apples.size(); i++) {
        pt d = apples[i] - apples[i - 1];
        if (sq(apples[i] - apples[i - 1]) > 1e-9)
            allSame = false;
    }
    if (allSame) {
